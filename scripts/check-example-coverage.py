...
def wasm_only(lines: list[str], number: int) -> bool:
    """Is the `pub fn` on line `number` reachable only from JavaScript?

    The `#[wasm_bindgen]` layer cannot run under a native example: it
    needs a JavaScript runtime to hand values to, which is why the
    crate keeps the deciding logic in `core` and the bindings thin.
    Demanding a native example for `JsError`-returning entry points
    would be demanding something impossible.

    The exclusion is per-function rather than per-file, so a plain
    `pub fn` added to `lib.rs` is still checked, and the count of
    exclusions is printed so it cannot grow unnoticed.
    """
    # Walk back over the function's own attributes and doc comments.
    i = number - 2
    while i >= 0:
        text = lines[i].strip()
        if text.startswith("#[") and "wasm_bindgen" in text:
            return True
        if text.startswith(("///", "//", "#[")) or not text:
            i -= 1
            continue
        break
    # Otherwise it may sit inside a `#[wasm_bindgen] impl` block.
    for j in range(number - 2, -1, -1):
        text = lines[j].strip()
        if text.startswith("impl "):
            for k in range(j - 1, -1, -1):
                above = lines[k].strip()
                if above.startswith("#[") and "wasm_bindgen" in above:
                    return True
                if not above.startswith(("#[", "///", "//")) and above:
                    break
            return False
        if text.startswith(("pub fn ", "fn ")) and j != number - 1:
            # Reached the previous item without crossing an `impl`.
            return False
    return False
...
def main() -> int:
    ...
    for path in sorted(SRC.rglob("*.rs")):
        ...
        declared = [(n, t) for n, t in candidates if not wasm_only(lines, n)]
        excluded += len(candidates) - len(declared)
    ...
    print(
        f"{total - len(unexercised)}/{total} public functions exercised "
        f"({excluded} JavaScript-only entry points excluded)"
    )
    if unexercised:
