...
fn query_text_crosses_the_boundary_as_an_array() {
    let doc = parse(DOC).expect("well-formed");
    let titles = doc.query_text("//title", None).expect("valid expression");
...
fn query_value_and_count_agree_with_the_document() {
    let doc = parse(DOC).expect("well-formed");
    assert_eq!(doc.query_value("count(//book)", None).expect("valid"), "2");
    assert_eq!(doc.query_count("//book", None).expect("valid"), 2);
    assert_eq!(doc.query_count("//missing", None).expect("valid"), 0);
}

#[wasm_bindgen_test]
fn a_malformed_expression_is_an_error_on_every_entry_point() {
    let doc = parse(DOC).expect("well-formed");
    assert!(doc.query_text("//[", None).is_err());
    assert!(doc.query_value("//[", None).is_err());
    assert!(doc.query_count("//[", None).is_err());
}
...
fn the_document_survives_a_failed_query() {
    ...
    let doc = parse(DOC).expect("well-formed");
    assert!(doc.query_text("//[", None).is_err());
    assert_eq!(doc.query_text("//title", None).expect("valid").len(), 2);
}
...
fn non_ascii_content_survives_the_boundary() {
    ...
    let text = doc.query_text("//b", None).expect("valid");
    assert_eq!(text, vec!["Germinal 😀 é 中".to_owned()]);
}

/// Namespace bindings cross the boundary as `"PREFIX=URI"` strings.
#[wasm_bindgen_test]
fn namespace_bindings_cross_the_boundary() {
    let doc = parse(
        r#"<r xmlns:m="urn:u"><m:item>ns</m:item><item>plain</item></r>"#,
    )
    .expect("well-formed");
    let bound = vec![String::from("m=urn:u")];
    assert_eq!(
        doc.query_text("//m:item", Some(bound)).expect("valid"),
        vec![String::from("ns")]
    );
    // Unprefixed matches only nodes in no namespace.
    assert_eq!(
        doc.query_text("//item", None).expect("valid"),
        vec![String::from("plain")]
    );
    // An unbound prefix is an error, not a silent match.
    assert!(doc.query_count("//m:item", None).is_err());
}
